...
#
# The environment
SERVER_IMAGE=thatsk/jmeter-server:v1
CLIENT_IMAGE=thatsk/jmeter-client:v1
DATADIR=
JMX_SCRIPT=
WORK_DIR=$(readlink -f /tmp)
NUM_SERVERS=1
HOST_WRITE_PORT=49502
HOST_READ_PORT=49503
	...
	do
	  ...
	  mkdir -p ${LOGDIR}
		...
		docker run --cidfile ${LOGDIR}/cid \
					...
					--name ${containername}-jmeter-server-${n} \
...
function stop_servers() {
  ...
  n=1
	while [[ ${n} -le ${NUM_SERVERS} ]]
	do
		docker stop ${containername}-${SERVER_NAME_PREFIX}-${n}
		n=$((${n}+1))
	done
...
function remove_containers() {
	...
	while [[ ${n} -le ${NUM_SERVERS} ]]
	do
		docker rm ${containername}-${SERVER_NAME_PREFIX}-${n}
		n=$((${n}+1))
	done
}
...
docker run --cidfile ${LOGDIR}/cid \
				...
				--name ${containername}-jmeter-client \
